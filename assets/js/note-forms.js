...
(function () {
  ...
  /* ---------- ready-made Assessment lines ----------
     L(label, line): the chip shows the short label — what the line is FOR — and tapping
     it inserts the full sentence into the editable box below. Showing the raw template
     on the chip made every generic line read as "__ (__): __; for __", which told the
     clinician nothing about what they were about to insert. */
  const L = (label, line) => ({ t: label, v: line });

  const A = {
    'Diabetes': [
      L('Controlled', 'T2DM (controlled, HbA1c __%): no change today'),
      L('Uncontrolled — adherence gap', 'T2DM (uncontrolled, HbA1c __%, rising from __%): adherence gap identified; for dietitian referral and recheck 3/12'),
      L('Uncontrolled — increase dose', 'T2DM (uncontrolled, HbA1c __%): for Metformin increased to __; recheck 3/12'),
      L('Overtreated — reduce', 'T2DM (overtreated, HbA1c __%): for dose reduction and recheck 6/52'),
      L('Foot screening normal', 'Diabetic foot screening: protective sensation intact, pulses palpable; no change today'),
      L('Eye screening overdue', 'Diabetic retinopathy screening overdue: for ophthalmology referral'),
    ],
    'Hypertension': [
      L('Controlled', 'HTN (controlled, __/__, home readings consistent): no change today'),
      L('Uncontrolled — increase dose', 'HTN (uncontrolled, __/__ despite __): for dose increase and home BP diary, recheck 4/52'),
      L('Uncontrolled — add second agent', 'HTN (uncontrolled, __/__): second agent added — __; renal profile and K in 2/52'),
      L('Postural drop — reduce', 'HTN with postural drop (__/__ lying, __/__ standing): for dose reduction given fall risk'),
    ],
    'Dyslipidaemia': [
      L('At target', 'Dyslipidaemia (LDL __ on __): at target; no change today'),
      L('Above target — intensify', 'Dyslipidaemia (LDL __, above target on __): for high-intensity statin; lipid panel 3/12'),
      L('Statin muscle symptoms', 'Statin-associated muscle symptoms (CK __): for washout 2/52 then rechallenge with __'),
    ],
    'Hypothyroidism': [
      L('Euthyroid', 'Hypothyroidism (TSH __ on Levothyroxine __ mcg): euthyroid; no change today'),
      L('TSH high — increase dose', 'Hypothyroidism (TSH __, above target): for Levothyroxine increased to __ mcg; TSH in 6/52'),
    ],
    'Asthma / COPD': [
      L('Asthma controlled', 'Asthma (controlled on __, no night waking, reliever __x/week): no change today'),
      L('Asthma uncontrolled — step up', 'Asthma (uncontrolled, __ of 4 control questions positive): technique checked and corrected; for step up to __, review 4/52'),
      L('COPD — frequent exacerbations', 'COPD (__ exacerbations in the last year): for inhaled therapy review and pulmonary rehabilitation referral'),
    ],
    'CKD': [
      L('Stable', 'CKD (eGFR __, was __ twelve months ago, ACR __): stable; for annual monitoring'),
      L('Declining — refer', 'CKD (eGFR __, declining): drug doses reviewed and adjusted; for nephrology referral'),
      L('Albuminuria — ACEi + SGLT2', 'CKD with albuminuria (ACR __): for ACE inhibitor/ARB titrated to maximum tolerated dose and SGLT2 inhibitor added'),
      L('Nephrotoxic drugs reviewed', 'CKD (eGFR __): nephrotoxic drugs reviewed — __ stopped; sick day rules explained'),
    ],
    'IHD / AF': [
      L('IHD stable', 'IHD (stable, no exertional symptoms, on __): no change today'),
      L('IHD worsening — refer', 'IHD with new or worsening exertional symptoms: for urgent cardiology review; secondary prevention optimised'),
      L('AF rate controlled', 'AF (rate controlled, __ bpm, CHA2DS2-VASc __, on __): no change today'),
      L('AF rate above target', 'AF (rate __ bpm, above target): for rate-control dose increase and repeat ECG'),
      L('AF not anticoagulated', 'AF with CHA2DS2-VASc __ and no anticoagulation: for anticoagulation started after bleeding risk discussion'),
      L('Secondary prevention complete', 'Secondary prevention reviewed: antiplatelet, statin, ACE inhibitor and beta blocker all in place'),
    ],
    'Osteoporosis': [
      L('On treatment, replete', 'Osteoporosis (T-score __, on __): tolerating treatment; calcium and vitamin D replete'),
      L('Fracture on treatment', 'Fragility fracture on treatment: for treatment review and DEXA'),
    ],
    'Obesity': [
      L('For weight management', 'Obesity (BMI __): for structured weight management; 5-10% target discussed'),
      L('Meets criteria for drug', 'Obesity (BMI __) with comorbidity: meets criteria for pharmacotherapy; for GLP-1 receptor agonist discussion'),
      L('For bariatric referral', 'Obesity (BMI __) after failed structured attempts: for bariatric surgery referral'),
      L('Weight improving', 'Weight stable/reduced by __ kg since last visit: for continued current plan'),
    ],
    generic: [
      L('Blank line', '__ (__): __; for __'),
      L('Stable — no change', '__ (stable): no change today'),
      L('Reassuring, no red flags', 'Reassuring history and examination, no red flags: for symptomatic management and safety netting'),
      L('Diagnosis still uncertain', 'Diagnostic uncertainty at this stage: for __ and reassessment in __'),
    ],
    defensive: [
      L('Avoided a drug or test', '__ avoided due to __'),
      L('No imaging indicated', 'No indication for imaging at this stage'),
      L('Patient insisted — I declined', '__: explained in detail, but still insisting; not ordered, rationale documented; __ offered and accepted'),
      L('Patient declined my advice', '__: declined after discussion; risks explained and documented, __ offered instead'),
    ],
  };
  ...
  /* Assessment lines for the complaint-driven visits. Each library covers the real
     ...
     workup, and the one that escalates. */
  const AV = {
    palpitation: [
      L('Benign — reassure', 'Palpitation (benign features, __ months, no red flags, ECG today normal): reassured; for TFT and CBC to exclude secondary causes, review with results'),
      L('Exertional — urgent referral', 'Palpitation (exertional, with __): features not consistent with a benign cause; for urgent cardiology referral and same-day ECG'),
      L('AF on ECG', 'Palpitation (irregular, ECG showing AF at __ bpm): for rate control and CHA2DS2-VASc-guided anticoagulation; echo requested'),
      L('Ectopic beats — benign', 'Palpitation (isolated ectopic beats on ECG, no structural features): benign ectopy; reassurance given, no further workup indicated'),
      L('Undiagnosed — Holter', 'Palpitation undiagnosed after initial workup: for 24-hour Holter monitoring; symptom diary explained'),
    ],
    'chest-pain': [
      L('Exertional — likely angina', 'Chest pain (exertional, relieved by rest, ECG today with no acute changes): features consistent with stable angina — a normal resting ECG does not exclude it; for urgent cardiology referral, aspirin and GTN started'),
      L('Musculoskeletal', 'Chest pain (reproducible on palpation, non-exertional, __ weeks): musculoskeletal in origin; for NSAID course and review 2/52'),
      L('Reflux', 'Chest pain (burning, postprandial, worse lying flat): consistent with reflux; for PPI trial 8/52'),
      L('Pleuritic — consider PE', 'Chest pain (pleuritic, Wells __): PE considered; for D-dimer and CTPA if positive'),
      L('Red flags — to ER', 'Chest pain with red flags (__): not safe for outpatient workup; sent to ER with handover given'),
      L('Atypical with risk factors', 'Atypical chest pain with multiple risk factors: ischaemia cannot be excluded on history alone; for functional or anatomical testing'),
    ],
    headache: [
      L('Migraine', 'Migraine without aura (meets criteria, __ attacks/month, no red flags): for acute treatment as below; prophylaxis discussed'),
      L('Medication overuse', 'Medication-overuse headache (analgesia on __ days/month): for structured withdrawal and prophylaxis started; worsening for two weeks explained'),
      L('Tension-type', 'Tension-type headache (bilateral, non-pulsating, no red flags): for simple analgesia; sleep and stress addressed'),
      L('Red flags — image', 'Headache with red flags (__): SNOOP positive; for urgent imaging and referral'),
      L('Over 50 — ? giant cell arteritis', 'New headache over age 50 with ESR __: giant cell arteritis suspected; for high-dose steroid started today and urgent referral'),
      L('Unclear — diary', 'Headache of unclear type: for a headache diary over 4/52 and reassessment'),
    ],
    'back-pain': [
      L('Mechanical — no imaging', 'Mechanical low back pain (__ weeks, no red flags, normal neurology): no indication for imaging at this stage; for analgesia, activity maintenance and physiotherapy'),
      L('Radiculopathy', 'Lumbar radiculopathy (dermatomal, SLR positive, power intact): for conservative management and review 4/52; refer if the deficit progresses'),
      L('Red flags — MRI', 'Low back pain with red flags (__): for urgent MRI and specialist referral'),
      L('? Cauda equina — ER', 'Suspected cauda equina syndrome (__): emergency; sent to ER directly and the spinal team informed'),
      L('Chronic — exercise programme', 'Chronic low back pain (over 12 weeks): for a structured exercise programme; opioids avoided given limited benefit and harm profile'),
    ],
    dizziness: [
      L('BPPV — Epley done', 'BPPV (positional, lasting seconds, Dix-Hallpike positive on the __ side): Epley manoeuvre performed today; home exercises explained'),
      L('Orthostatic — drug induced', 'Orthostatic hypotension (__/__ lying to __/__ standing): drug-induced; for __ reduced and postural BP rechecked 2/52'),
      L('Vestibular neuritis', 'Vestibular neuritis (continuous over days, no central signs): for a short vestibular sedative course and vestibular rehabilitation'),
      L('Central features — ER', 'Dizziness with central features (__): posterior circulation stroke cannot be excluded; sent to ER'),
      L('Presyncope — cardiac', 'Presyncope with cardiac features: for ECG, postural BP and cardiology referral'),
    ],
    fatigue: [
      L('Sleep deficit', 'Fatigue (__ months, identified sleep deficit, no B symptoms, examination normal): most consistent with sleep insufficiency; for sleep hygiene and reassessment 4/52'),
      L('Iron deficiency', 'Fatigue with iron deficiency (Hb __, ferritin __): for iron replacement and investigation of the cause'),
      L('Hypothyroid', 'Fatigue with hypothyroidism (TSH __): for levothyroxine started and TSH in 6/52'),
      L('Depression screen positive', 'Fatigue with a positive depression screen (PHQ-9 __): for treatment as below'),
      L('Red flags — urgent workup', 'Fatigue with red flags (__): malignancy and infection not excluded; for urgent workup'),
      L('Workup negative', 'Fatigue with a negative workup: no organic cause identified; for graded activity and review 3/12'),
    ],
    'sore-throat': [
      L('Viral — no antibiotic', 'Acute pharyngitis (Centor __, viral features): no antibiotic indicated; symptomatic treatment and safety netting given'),
      L('Strep — antibiotic', 'Streptococcal pharyngitis (Centor __, rapid test positive): for amoxicillin 10 days; the reason for completing the course explained'),
      L('Airway red flags — refer', 'Pharyngitis with airway red flags (__): peritonsillar abscess or epiglottitis considered; referred same day'),
      L('? Mononucleosis', 'Suspected infectious mononucleosis (posterior nodes, __): for monospot and CBC; amoxicillin avoided and contact sport restriction advised'),
    ],
    dysuria: [
      L('Uncomplicated cystitis', 'Uncomplicated cystitis (typical symptoms, dipstick __): for nitrofurantoin 5 days; culture sent'),
      L('Pyelonephritis', 'Pyelonephritis (fever, flank pain, __): for oral ciprofloxacin 7 days and culture; admission criteria explained'),
      L('Asymptomatic — no antibiotic', 'Asymptomatic bacteriuria: no antibiotic indicated — treatment carries harm without benefit'),
      L('Recurrent UTI', 'Recurrent UTI (__ episodes this year): for post-void residual measurement; topical vaginal oestrogen discussed'),
      L('Haematuria — urology', 'Dysuria with visible haematuria: for urgent urology referral — malignancy must be excluded'),
    ],
    'abdo-pain': [
      L('Dyspepsia', 'Dyspepsia (epigastric, no alarm features, age __): for a PPI trial 8/52 and H. pylori testing'),
      L('Biliary colic', 'Biliary colic (right upper quadrant, postprandial): for abdominal ultrasound and surgical referral'),
      L('IBS', 'IBS meeting Rome IV criteria with no red flags: a positive diagnosis, made and explained; for dietary measures and an antispasmodic'),
      L('Peritonism — ER', 'Abdominal pain with peritonism (__): surgical abdomen; sent to ER'),
      L('Pregnancy considered', 'Abdominal pain in a woman of reproductive age: pregnancy test done today (__); ectopic pregnancy considered'),
    ],
    'joint-pain': [
      L('Osteoarthritis', 'Osteoarthritis (mechanical pattern, morning stiffness under 30 minutes): for paracetamol, topical NSAID and physiotherapy; oral NSAIDs avoided due to __'),
      L('? Inflammatory — refer', 'Inflammatory arthritis suspected (symmetrical, stiffness over 30 minutes, __ joints): for RF, anti-CCP and inflammatory markers with early rheumatology referral'),
      L('Acute gout', 'Acute gout (__ joint, urate __): for a short anti-inflammatory course; urate-lowering therapy discussed'),
      L('Hot joint — aspirate', 'Hot swollen joint with fever: septic arthritis cannot be excluded; referred for urgent aspiration'),
      L('Tendinopathy', 'Tendinopathy / soft tissue injury (__): for relative rest and a progressive loading programme'),
    ],
    dyspnea: [
      L('Asthma exacerbation', 'Asthma exacerbation (__ of 4 control questions positive): for prednisolone 5 days and step-up; inhaler technique checked and corrected'),
      L('COPD exacerbation', 'COPD exacerbation (increased sputum purulence): for prednisolone and an antibiotic, 5 days each'),
      L('Bronchitis — no antibiotic', 'Acute bronchitis (viral features, no focal signs): no antibiotic indicated; expected 2-3 week cough explained'),
      L('Pneumonia', 'Community-acquired pneumonia (focal signs, CRB-65 __): for amoxicillin and a chest X-ray'),
      L('? Heart failure', 'Dyspnoea with cardiac features (orthopnoea, PND, oedema): heart failure suspected; for BNP, ECG and echo'),
      L('Chronic cough', 'Chronic cough over 8 weeks: for chest X-ray; ACE inhibitor, reflux and asthma each considered'),
    ],
    falls: [
      L('Recurrent — multifactorial', 'Recurrent falls (__ in __ months, Timed Up and Go __ seconds): multifactorial with a dominant drug contribution; for medication review, physiotherapy and home hazard assessment'),
      L('With LOC — this is syncope', 'Fall with loss of consciousness: this is syncope rather than a mechanical fall; for ECG, postural BP and cardiology referral'),
      L('Single, no injury', 'Single mechanical fall, no injury, gait and balance normal: for exercise advice and rescreening in 12/12'),
      L('Head injury on anticoagulant', 'Fall with head injury on anticoagulation: for CT head regardless of how well the patient appears'),
      L('? Fracture', 'Fall with suspected fracture (__): for X-ray and orthopaedic referral; osteoporosis treatment indicated regardless of DEXA'),
    ],
    memory: [
      L('Cognitive impairment', 'Cognitive impairment (Mini-Cog __/5, informant-corroborated functional decline over __): for a reversible cause screen and neuroimaging, with memory clinic referral'),
      L('Subjective only — normal', 'Subjective memory complaint with normal testing and no functional decline: reassurance given; for review 6/12'),
      L('Acute onset — ? delirium', 'Cognitive change of acute onset: delirium until proven otherwise; for infection screen and medication review'),
      L('? Lewy body', 'Cognitive impairment with visual hallucinations and parkinsonism: Lewy body dementia considered — antipsychotics avoided'),
      L('Driving concern', 'Cognitive impairment with ongoing driving: driving discussed and documented; advised to notify the licensing authority'),
    ],
    routine: [
      L('Routine check', 'Routine health check, asymptomatic: for age-appropriate screening as below'),
      L('CVD primary prevention', 'Cardiovascular primary prevention (ASCVD 10-year risk __%): for a statin discussion; lifestyle measures reinforced'),
      L('Screening up to date', 'Screening up to date and no new issues identified: no change today'),
      L('Prediabetes', 'Prediabetes (HbA1c __%): for a structured lifestyle intervention; HbA1c in 6/12'),
      L('Immunisation', 'Immunisation reviewed: __ given today; remainder up to date'),
    ],
  };
